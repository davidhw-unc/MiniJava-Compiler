...
public class Interpreter {
    ...
    final static int running = 0, halted = 1, failedDataStoreFull = 2, failedInvalidCodeAddress = 3,
            ...
            failedIOError = 7, failedArrayIndex = 8, failedNullRef = 9, failedHeapRef = 10,
    ...
    static void dump() {
        ...
        if (ST == SB)
            ...
            for (addr = ST - 1; addr >= SB; addr--) {
                ...
                else System.out.print("|" + leftPad(8, String.valueOf(data[addr])) + "|");
                System.out.println("");
                if (addr == dynamicLink) {
                    System.out.println("            |--------|");
    ...
    static void checkSpace(int spaceNeeded) {
        ...
        // heap by spaceNeeded.
        if (HT - ST < spaceNeeded) status = failedDataStoreFull;
    }
    ...
    static boolean invalidHeapRef(int addr) {
        ...
        else if (addr < HT + 2 || addr > HB) status = failedHeapRef;
    ...
    static int readInt() throws java.io.IOException {
        ...
        if ((currentChar == '-') || (currentChar == '+')) do {
            sign = (currentChar == '-') ? -1 : 1;
            currentChar = System.in.read();
        } while ((currentChar == '-') || currentChar == '+');

        if (Character.isDigit((char) currentChar)) do {
    ...
    static void callPrimitive(int id) {
        ...
        switch (prim) {
            ...
            case arraylen:
                ...
                addr = data[ST - 1];
                if (invalidHeapRef(addr)) break; // invalid heap address
                if (data[addr - 2] != -2) {
    ...
    static void interpretOneOperation() {
        ...
        switch (operation) {
            ...
            case RETURN:
                ...
                CP = data[LB + 2];
                LB = data[LB + 1];
                if (n == 1) data[addr] = data[ST - 1]; // return value if any
            ...
            // arguments on stack, followed by instance addr
            {
                ...
                if (classDescAddr >= ST || classDescAddr <= SB || d >= data[classDescAddr + 1]
                        || d < 0) {
    ...
    static void printHelp() {
        String[] help = { "p or print:", "     print entire machine state",
                "l or list [offset] [size]:",
                "     print the instructions around CP + offset, with size lines on either side",
                "     offset = 0 and size = 2 by default", "b or break [address]:",
                "     set a breakpoint at address", "     address = CP by default", "del:",
                "     delete one or more breakpoints", "n or next:", "     execute one instruction",
                "c or continue:",
                "     continue running the program from current position, until next breakpoint or completion",
                "r or run:", "     run the program from start, until next breakpoint or completion",
                "i or info:", "     list the current breakpoints", "q, quit or <EOF>:",
                "     quit the debugger", "Simply press enter to repeat the last command",
    ...
    static void debugProgram() {
        ...
        while (true) {
            ...
            if (command.equals("?") || command.equals("help")) {
                ...
                int offset = 0, size = 2;
                if (scanner.hasNextInt()) offset = scanner.nextInt();
